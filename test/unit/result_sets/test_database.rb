...
class UnitTests::TestResultSets::TestDatabase < Test::Unit::TestCase
  test "with default options" do
    ...
    Linkage::ScoreSets::Database.expects(:new).with({:conn => database}).returns(score_set)
    assert_same score_set, result_set.score_set

    match_set = stub('match set')
    Linkage::MatchSets::Database.expects(:new).with({:conn => database}).returns(match_set)
    assert_same match_set, result_set.match_set
  end

  test "with database object" do
    database = stub('database')
    result_set = Linkage::ResultSets::Database.new({:conn => database})
  ...
  test "with overwrite option" do
    ...
    score_set = stub('score set')
    Linkage::ScoreSets::Database.expects(:new).with({:conn => database, :overwrite => true}).returns(score_set)
    assert_same score_set, result_set.score_set

    match_set = stub('match set')
    Linkage::MatchSets::Database.expects(:new).with({:conn => database, :overwrite => true}).returns(match_set)
    assert_same match_set, result_set.match_set
  end
